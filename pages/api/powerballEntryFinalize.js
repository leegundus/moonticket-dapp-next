...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!RPC_URL || !SUPABASE_URL || !SUPABASE_KEY || !TIX_MINT) {
    ...
    // If a payment was expected, verify it on-chain
    if (Number(expectedTotalBase) > 0) {
      ...
      // Compute how much TIX the treasury ATA gained in this tx
    ...
    // Get latest draw (credits are per-draw)
    ...
    // Build entries: first N are credit-backed with their original type
    const creditCount = Number(lockedCredits);
    const creditEntries = tickets.slice(0, creditCount).map((t, i) => ({
      wallet,
      entry_type: creditTypes[i] || "promo",
      num1: t.num1, num2: t.num2, num3: t.num3, num4: t.num4, moonball: t.moonball,
    }));
    const paidEntries = tickets.slice(creditCount).map(t => ({
      wallet,
      entry_type: "purchase",
      num1: t.num1, num2: t.num2, num3: t.num3, num4: t.num4, moonball: t.moonball,
    }));
